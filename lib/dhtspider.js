...
DHTSpider.prototype.onAnnouncePeerRequest = function(msg, rinfo) {
    ...
    //更新最近两周热度
    db.update("call updatehots_pro(?)",[infohash.toString('hex')],function(err,result) {
        if(err) {
            return console.error(err);
        }
        var now = moment().format('YYYY-MM-DD HH:mm:ss');
        db.update("call updatehistory_pro(?,?,?,?)",[infohash.toString('hex'),rinfo.address,port,now],function(err,result) {
            if(err) {
                return console.error(err);
            }
        });
